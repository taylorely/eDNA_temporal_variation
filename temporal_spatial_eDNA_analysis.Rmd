...
ggsave(filename = here("figures","Fig1.tiff"),
       plot = fig1,
       dpi = 600,
       width = 7.7,
  height = 4.5,
...
jpeg(here("figures","FigS2.tiff"), width=7, height=4.5, units = "in", res=600)
...
#Fig 2. Heat map of monitored species ordered by detection
```{r}
...
#make figure
jpeg(here("figures","Fig2.tiff"), width=7.5, height=4.75, units="in",res=600)
...
### Figure S1: Heat Map of all taxa 
```{r}
...
#make the figure
jpeg(here("figures","FigS1.tiff"), width=7.5, height=6.25, units="in", res=600)
...
## Aportioned Variance plot (make figure)
...
col.great4 <- c(col3.gr[2],col2.gr[2],col_Zissou1[1],col3.gr[5])

#Plot Figure 
jpeg(here("figures","Fig3.tiff"),  width=7.5, height=4,units="in", res=600)
...
# Look into detection of species richness with only spatial replicates vs only temporal replicates for both monitored subset and all taxa. This is in the results section under the subheader: "Temporal and spatial variation in native eDNA signatures
```{r}
...
## Monitored Taxa means for temporal replicates
```{r}
...
# calculate the mean between the three locations for three samples
temporal.mean.mon<-c(out.mon.temporal$iNextEst$A$qD[3],out.mon.temporal$iNextEst$B$qD[3],out.mon.temporal$iNextEst$C$qD[3])
mean(temporal.mean.mon) 

# calculate the lower limit for the three locations for three samples
...
mean(temporal.min.mon) 

# calculate the upper limit for three locations for three samples
...
mean(temporal.max.mon) 

```

## Monitored Taxa means for spatial replicates
```{r}
...
#mean of spatial replicates for three samples
...
mean(spatial.mean.mon)

# calculate the lower limit for three samples
...
mean(lower.spatial.mon) 

# calculate the upper limit for three samples
...
mean(upper.spatial.mon) 
```

## All Taxa species detection with temporal replicates
```{r}
...
# calculate mean for temporal replicates for three samples for all taxa
temporal.mean<-c(out.temporal$iNextEst$A$qD[[3]],
                 out.temporal$iNextEst$B$qD[[3]],
                 out.temporal$iNextEst$C$qD[[3]])
mean(temporal.mean) 

# calculate lower limit for temporal replicates for three samples for all taxa
temporal.min<-c(out.temporal$iNextEst$A$qD.LCL[[3]],
                 out.temporal$iNextEst$B$qD.LCL[[3]],
                 out.temporal$iNextEst$C$qD.LCL[[3]])
mean(temporal.min) 

#calculate upper limit for temporal replicates for three samples for all taxa
...
## All taxa species detection with spatial replicates
```{r}
...
# calculate mean of spatial replicates for three replicates
...
# calculate lower limit of spatial replicates for three replicates
...
# calculate upper limit of spatial replicates for three replicates
...
jpeg(here("figures","Fig4.tiff"),  width=7, height=4.75, units = "in", res=600)
...
# Figure S3A NMDS with direction of tide
```{r}
#NMDS Bray with tide
...
jpeg(here("figures","FigS3A.tiff"), width=7, height=5.5, units="in", res=600)
...
# Figure S3B NMDS with temporal replicates
```{r}
#NMDS Bray with time
jpeg(here("figures","FigS3B.tiff"), width=7, height=5.5, units= "in", res=600)
